        ...
        return (
            <tr>
                ...
                <td>{singleRow.status < 0 ? (
                    <div>
                    <input type="button" className="btn btn-primary" value="Save" onClick={() => this.submitForm(index)} />
                    &nbsp;
                    <input type="button" className="btn btn-default" value="Cancel" onClick={() => updateStatus(index, 1)} />
                    </div>) : (

                    <div>
                        <button className="btn btn-warning" onClick={() => this._updateStatus(index, -1)}>Edit</button>
                         &nbsp;
